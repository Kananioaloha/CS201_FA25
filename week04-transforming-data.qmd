...
author: "Amber Camp & Connor Flynn & Kanani"
date: 9/15/25
format: html
editor: visual
---
...
Let's make the same faceted scatter plot from the new dataframe. Can you make it interactive using `plotly`?

```{r, message=FALSE}
mass_plot <- ggplot(data = penguins_male_female, aes(x = flipper_length_mm, y = body_mass_g, color = species, shape = island)) +
  geom_point() +
  theme_bw() +
  scale_color_viridis_d() +
  facet_wrap(~sex) + # here is where we designate faceting!
...
average_flipper_length <- penguins_data %>% 
  group_by(species) %>% 
  summarise(average_flipper_length = mean(body_mass_g, na.rm = TRUE))
...
ggplot(average_culmen_depth, aes(x = species, y = average_culmen_depth)) +
  geom_col(fill = "pink", color = "black") +
  labs(
    title = "Average Culmen Depth by Species",
    x = "Species",
    y = "Average culmen depth (mm)"
  ) +
  theme_bw()

ggplot(average_culmen_depth, aes(x = species, y = average_culmen_depth, fill = species)) +
  geom_col(color = "black") +
  scale_fill_manual(values = c("Adelie" = "pink1", "Chinstrap" = "pink3", "Gentoo" = "deeppink")) +
  theme_bw() +
  guides(fill = "none")
```
