...
```{r}
teds_d <- teds_d %>%
  mutate(services_d = factor(recode(as.character(services_d),
    "1"="Detox 24hr hospital",
    "2"="Detox 24hr residential",
    "3"="Rehab inpatient hospital",
    "4"="Rehab short ≤30d",
    "5"="Rehab long >30d",
    "6"="Ambulatory intensive",
    "7"="Ambulatory non-intensive",
    "8"="Ambulatory detox",
    .missing="unknown"
  )))
```
...
```{r}
teds_d <- teds_d %>%
  mutate(reason = factor(recode(as.character(reason),
    "1"="completed",
    "2"="transferred",
    "3"="left AMA",
    "4"="terminated",
...
```{r}
ggplot(teds_d, aes(x = freq1_d, fill = freq1_d)) +
  geom_bar(show.legend = FALSE) +
  labs(title = "Frequency of Use at Discharge", x = NULL, y = "Count") +
  theme_minimal()
```

```{r}
ggplot(teds_d, aes(x = reason, fill = reason)) +
  geom_bar(show.legend = FALSE) +
  labs(title = "Reason for Discharge", x = NULL, y = "Count") +
  theme_minimal() +
...
```{r}
ggplot(teds_d, aes(x = services_d, fill = freq1_d)) +
  geom_bar() +
  labs(title = "Service Type by Frequency of Use at Discharge",
       x = NULL, y = "Count", fill = "Freq") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
```{r}
library(here); library(arrow); library(janitor); library(dplyr)

# make sure data/ exists
dir.create(here("data"), showWarnings = FALSE)

# read the lecture file, clean names, assign to teds_d
teds_d <- read_parquet(here("data", "teds_d_lecture.parquet")) %>%
  clean_names()

# (optional: do your recodes here)

# save the clean file for Wednesday
write_parquet(teds_d, here("data", "teds_d_clean.parquet"))
