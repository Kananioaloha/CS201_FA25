...
penguins_no_comments <-penguins %>%
  select(-comments)

view(penguins_no_comments)
```
...
mutate <- penguins %>%
  dplyr::mutate(flippometers = flipper_length_mm * 0.000001)
...
mutate(
  ...
  summarize(
    avg_mass_g = round(mean(body_mass_g, na.rm = TRUE), 0),
    .groups = "drop"
  ) %>%
  arrange(species, island, flipper_rank)
