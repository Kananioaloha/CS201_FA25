...
# Gentoo penguins from Palmer LTER (PASTA repository)
gentoo_penguins_palmer_lter <- read_csv(
  "https://pasta.lternet.edu/package/data/eml/knb-lter-pal/220/3/e03b43c924f226486f2f0ab6709d2381",
  na = c("", "NA", "NaN", "."),
  show_col_types = FALSE
)

# Quick check
glimpse(gentoo_penguins_palmer_lter)

# Optional: make column names easier to use (snake_case)
# install.packages("janitor")  # if needed
# library(janitor)
# gentoo_penguins_palmer_lter <- clean_names(gentoo_penguins_palmer_lter)
# names(gentoo_penguins_palmer_lter)
```
...
```{r, message=FALSE}
# names(penguins_data)[names(penguins_data) == "Culmen Length (mm)"] <- culmen_length_mm"" "culmen_length_mm"
```
...
ggplot(data = penguins_data, aes(x = flipper_length_mm, fill = species)) +
  geom_histogram(alpha = 0.5) + #alpha adds transparency
  theme_bw() +
